...
import { Search, Lightbulb, Cloud, Leaf, Bell, Heart } from "lucide-react";
...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    getReminders().then((list) => {
      setDueReminders(
        list.filter((r) => r.enabled && getDaysUntil(r.nextDate) <= 1)
      );
    });
  }, []);
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card
          className="p-4 cursor-pointer hover:shadow-md transition-shadow bg-gradient-to-br from-green-50 to-emerald-50 border-green-100"
          onClick={() => navigate("/my-plants")}
        >
          <div className="flex items-center gap-2 text-green-700">
            <Heart className="w-5 h-5" />
            <span className="text-xs font-medium">Coleção</span>
          </div>
          <p className="text-2xl font-bold text-gray-800 mt-2">{myPlants.length}</p>
          <p className="text-xs text-gray-500">
            {myPlants.length === 1 ? "planta" : "plantas"}
          </p>
        </Card>
        <Card
          className={`p-4 cursor-pointer hover:shadow-md transition-shadow ${
            dueReminders.length > 0 || needsWaterCount > 0
        ...
        >
          <div className="flex items-center gap-2 text-orange-700">
            ...
            <span className="text-xs font-medium">Hoje / amanhã</span>
          </div>
          <p className="text-2xl font-bold text-gray-800 mt-2">
            {Math.max(dueReminders.length, needsWaterCount)}
          </p>
          <p className="text-xs text-gray-500">
            {dueReminders.length + needsWaterCount === 0
              ? "tudo em dia"
              : "cuidados pendentes"}
          </p>
        </Card>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-semibold text-gray-800">Lembretes de cuidado</h4>
            <p className="text-sm text-gray-600">
              Marque regas e fertilizações para não esquecer
            </p>
          </div>

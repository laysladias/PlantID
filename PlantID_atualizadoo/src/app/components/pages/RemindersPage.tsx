...
export function RemindersPage() {
  ...
  // e escolhe entre as plantas que batem com o texto, em vez de rolar uma
  // lista enorme com todas as plantas cadastradas.
  const [plantSearch, setPlantSearch] = useState("");
  const [showPlantSuggestions, setShowPlantSuggestions] = useState(false);

  const filteredPlants = useMemo(() => {
    const term = plantSearch.trim().toLowerCase();
    if (!term) return plants.slice(0, 8);
    return plants.filter((p) => p.name.toLowerCase().includes(term)).slice(0, 8);
  }, [plants, plantSearch]);
  ...
  const handleSelectPlant = (plant: Plant) => {
    setNewReminder((prev) => ({ ...prev, plantId: plant.id }));
  ...
  const handleMarkDone = async (id: string, type: string) => {
    const { error } = await markReminderDone(id, type === "watering" ? 7 : 30);
    if (error) {
      toast.error("Não foi possível atualizar.");
      return;
    }
    toast.success(
      type === "watering" ? "Rega registrada! Próximo aviso em ~7 dias." : "Fertilização registrada!"
    );
    loadReminders();
  };
  ...
  const handleAddReminder = async () => {
    if (!newReminder.plantId || !newReminder.frequency) {
      toast.error("Escolha uma planta pela busca antes de continuar.");
      return;
    }
          ...
          <DialogContent>
            ...
            <div className="space-y-4 mt-4">
              <div className="space-y-2 relative">
                <Label htmlFor="plant-search">Planta</Label>
                <Input
                  id="plant-search"
                  ...
                  onChange={(e) => {
                    ...
                    if (newReminder.plantId) {
                      setNewReminder((prev) => ({ ...prev, plantId: "" }));
                    }
                  }}
            ...
            {todayReminders.map((reminder) => {
              ...
              return (
                ...
                >
                  ...
                  <Button
                    size="sm"
                    onClick={() => handleMarkDone(reminder.id, reminder.type)}

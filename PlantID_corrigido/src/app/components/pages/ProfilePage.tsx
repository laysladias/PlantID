...
import { Mail, LogOut, Info, Camera } from "lucide-react";
...
export function ProfilePage() {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const img = new Image();
      img.onload = () => {
        // Recorta a imagem num quadrado centralizado (crop, não esticamento)
        // para que a foto de perfil nunca fique achatada/distorcida.
        const size = Math.min(img.width, img.height);
        const sx = (img.width - size) / 2;
        const sy = (img.height - size) / 2;

        const canvas = document.createElement("canvas");
        const outputSize = 320; // resolução suficiente para um avatar nítido
        canvas.width = outputSize;
        canvas.height = outputSize;
        const ctx = canvas.getContext("2d");

        if (!ctx) {
          // Fallback: se o canvas não estiver disponível, usa a imagem original

...
import { Plus, Droplet, Sun, AlertTriangle, Trash2 } from "lucide-react";
import { Card } from "../ui/card";
import { Button } from "../ui/button";
import { toast } from "sonner";
import { getMyPlants, waterMyPlant, deleteMyPlant, MyPlant } from "../../lib/db";
...
export function MyPlantsPage() {
  ...
  const handleWater = async (plantId: string) => {
    const { error } = await waterMyPlant(plantId);
    if (error) {
      toast.error("Não foi possível registrar a rega.");
      return;
    }
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {needsWater ? (
                        ...
                        <div className="text-xs text-gray-600">
                          Regar em {daysUntil} {daysUntil === 1 ? "dia" : "dias"}
                        </div>
                      )}
                    </div>

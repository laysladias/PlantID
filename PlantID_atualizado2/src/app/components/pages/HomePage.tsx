...
export function HomePage() {
  ...
  // Conta cuidados pendentes sem contar duas vezes a mesma planta: um
  // lembrete vencido ligado a uma planta da coleção e a própria planta
  // "atrasada" são a mesma pendência, então usamos um Set por identidade.
  const pendingIds = new Set<string>();
  dueReminders.forEach((r) => pendingIds.add(r.userPlantId ?? `reminder-${r.id}`));
  myPlants.forEach((p) => {
    if (p.nextWatering && getDaysUntil(p.nextWatering) <= 0) {
      pendingIds.add(p.id);
    }
  });
  const pendingCareCount = pendingIds.size;
